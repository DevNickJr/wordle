...
const Home = () => {
  ...
  const [triedletters, updateTriedLetters] = useReducer(
    (
      prevState: {
        wrongLetters: Array<string>;
        semiCorrectLetters: Array<string>;
        correctLetters: Array<string>;
      },
      newValue: string,
    ) => {
      return {
        wrongLetters: prevState.wrongLetters.concat(
          newValue.split("").filter((letter) => !answer.includes(letter)),
        ),
        semiCorrectLetters: prevState.semiCorrectLetters.concat(
          newValue.split("").filter((letter) => answer.includes(letter)),
        ),
        correctLetters: newValue
          .split("")
          .map((letter, index) =>
            answer[index] === letter
              ? letter
              : prevState?.correctLetters[index] ?? "",
          ),
      };
    },
    {
